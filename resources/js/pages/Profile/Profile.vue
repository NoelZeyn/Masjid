<template>
  <div class="flex h-screen bg-gray-100">
    ...
    <div class="flex-1 flex flex-col bg-white px-8 pt-3 md:px-6 pb-8">
            ...
            <div class="space-y-4">
              <div class="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-5">
                ...
                <input type="text" v-model="formData.nama_lengkap"
                       class="w-full bg-gray-50 border border-gray-300 rounded-md px-4 py-2 h-10" readonly />
              </div>
              ...
              <div class="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-5">
                <label class="md:w-[150px] font-bold text-sm text-gray-900">Email</label>
                <input type="email" v-model="formData.email"
                       class="w-full bg-gray-50 border text-gray-500 border-gray-300 rounded-md px-4 py-2 h-10" disabled />
              </div>
    ...
    async fetchProfile() {
      ...
      try {
        ...
        if (response.data.status === "success") {
          ...
          this.profileImage = user.foto_profil ? user.foto_profil : null;
        } else {
