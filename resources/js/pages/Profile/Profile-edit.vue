  ...
  <div class="flex min-h-[50vh] bg-gray-100">
    ...
    <div class="flex-1 p-8 pt-4 bg-white">
      ...
      <div class="bg-white border border-gray-300 rounded-lg shadow mt-6">
        <div class="p-6">
          <h3 class="text-sm font-bold text-gray-900 border-b pb-4">Edit Profil Pengguna</h3>
          ...
          <form @submit.prevent="submitProfile" class="mt-4 space-y-4">
            <div class="flex flex-col sm:flex-row items-center gap-4">
              ...
              <input v-model="form.nama_lengkap" type="text" required class="flex-1 p-2 border border-gray-300 rounded-md bg-gray-50" />
            </div>

            <div class="flex flex-col sm:flex-row items-center gap-4">
              <label class="w-full sm:w-40 text-sm font-semibold text-gray-700">Email</label>
              <input v-model="form.email" type="email" disabled readonly class="flex-1 p-2 text-gray-500 border border-gray-300 rounded-md bg-gray-50" />
            </div>

            <div class="flex flex-col sm:flex-row items-center gap-4">
              <label class="w-full sm:w-40 text-sm font-semibold text-gray-700">Kontak</label>
              <input v-model="form.kontak" type="text" class="flex-1 p-2 border border-gray-300 rounded-md bg-gray-50" />
            </div>

            <div class="flex flex-col sm:flex-row items-center gap-4">
              <label class="w-full sm:w-40 text-sm font-semibold text-gray-700">Foto Profil</label>
              <input type="file" @change="handleFileChange" accept="image/*" class="flex-1 p-2 flex-1 p-2 border border-gray-300 rounded-md bg-gray-50" />
            </div>

            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mt-6">
              <button type="submit" class="cursor-pointer px-8 py-2 text-white bg-[#074a5d] border border-[#074a5d] rounded-full font-medium hover:bg-gray-100 hover:text-black focus:outline-none focus:ring-2 focus:ring-[#074a5d]">
                Simpan Perubahan
              </button>
              <router-link to="/profile" class="cursor-pointer px-8 py-2 text-white bg-[#074a5d] border border-[#074a5d] rounded-full font-medium hover:bg-gray-100 hover:text-black focus:outline-none focus:ring-2 focus:ring-[#074a5d]">
                Batal
              </router-link>
            </div>
          </form>
